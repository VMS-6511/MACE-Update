...
echo "STARTING EVALUATION OF FINETUNED MODEL"
EVAL_FINETUNE_JOB_OUTPUT=$(./slurm/scripts/evaluation/launch_compute_metrics_finetune_job.sh $CUDA_VISIBLE_DEVICES $ALGO_NAME $CHANGE $TASK $CONFIG $PORT_NUMBER $PROMPTS_CSV $METRIC $RANDOM_SEED)
EVAL_FINETUNE_JOB_ID=$(echo "$EVAL_FINETUNE_JOB_OUTPUT" | grep -oP "Submitted batch job \K\d+")
echo "Waiting for evaluation of finetuned model job $EVAL_FINETUNE_JOB_ID to complete..."
srun --dependency=afterany:$EVAL_FINETUNE_JOB_ID --ntasks=1 --cpus-per-task=1 --partition=healthyml --qos=healthyml-main --account=healthy-ml --time=1-00:00 sleep 1
echo "Evaluation of finetuned model job $EVAL_FINETUNE_JOB_ID has completed."
