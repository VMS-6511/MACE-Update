...
if [ "$METRIC" == "FID" ]; then
    ...
    CUDA_VISIBLE_DEVICES=$1 python $PREFIX/evaluation/evaluate_fid.py --dir1 /data/healthy-ml/scratch/$(whoami)/projects/MACE-Update/experiments/${ALGO_NAME}/${CHANGE}_${TASK}_${CONFIG}/inference/${PROMPTS_CSV}/ --dir2 '/data/healthy-ml/scratch/$(whoami)/projects/MACE-Update/data/mscoco-30k'
    ...
    FINETUNE_DIR="$PREFIX/experiments/${ALGO_NAME}/${CHANGE}_${TASK}_${CONFIG}/finetune/lora/celebrity_celebrity_finetune_concepts/inference"
    ...
    CUDA_VISIBLE_DEVICES=$1 python $PREFIX/evaluation/evaluate_by_GCD.py --image_folder $PREFIX/experiments/${ALGO_NAME}/${CHANGE}_${TASK}_${CONFIG}/inference/${PROMPTS_CSV}/erased --save_excel_path $PREFIX/experiments/${ALGO_NAME}/${CHANGE}_${TASK}_${CONFIG}/results/${PROMPTS_CSV}/erased
    CUDA_VISIBLE_DEVICES=$1 python $PREFIX/evaluation/evaluate_by_GCD.py --image_folder $PREFIX/experiments/${ALGO_NAME}/${CHANGE}_${TASK}_${CONFIG}/inference/${PROMPTS_CSV}/others --save_excel_path $PREFIX/experiments/${ALGO_NAME}/${CHANGE}_${TASK}_${CONFIG}/results/${PROMPTS_CSV}/others
