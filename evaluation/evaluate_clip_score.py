...
def mean_clip_score(image_dir, prompts_path, results_file):
    ...
    all_entries = all_entries = os.listdir(image_dir)
    image_types=[entry for entry in all_entries if os.path.isdir(os.path.join(image_dir, entry))]
    if len(image_types) > 0:
        for image_type in image_types:
            if "type" in text_df.columns:
        ...
        texts = list(text_df['prompt'])
        image_filenames=[image for image in os.listdir(image_dir) if '.png' in image]
        print(len(texts), len(image_filenames))
        # assert len(texts)==len(image_filenames), "Number of images and prompts don't match"
        ...
        print('\n')
        print(f"Mean CLIP score ± Standard Deviation: {mean_similarity:.4f}±{std_similarity:.4f}")
        row = {'metric': 'CLIP Score', 'value': f"{mean_similarity:.4f}±{std_similarity:.4f}"}
        with open(results_file, 'a', newline='') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=row.keys())
            writer.writerow(row)  # write the data row   
